...
# 訓練模型的函數
def train_model(dataloader, model, criterion, optimizer, num_epochs=10,save_path="key_head_detector.pth"):
    for epoch in range(num_epochs):
        ...
        print(f"紀元 {epoch+1}/{num_epochs}, 平均損失: {avg_loss:.4f}")
        # 每個紀元結束後保存模型
...
def train():
    # 設定訓練數據路徑
    train_data_dir = "data/train/H648"
    ...
    train_dataset = KeyDataset(train_data_dir, transform=transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ]), use_labels=False)  # 設置為False來使用自監督學習
